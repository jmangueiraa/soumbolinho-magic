 ...
 * Usado como garantia de 100% de disponibilidade no cliente (offline/dev/fallback).
 */
...
export async function generateProductWithAi(
  ...
  const isDigital = params.isDigital !== undefined ? params.isDigital : false;
  ...
  // 2. Fallback local instantâneo multi-nichos com garantia de 100% de disponibilidade
  const localCopy = generateLocalSalesCopy(cleanName, params.category, isDigital);
  return {
    success: true,
    ...localCopy,
  };
