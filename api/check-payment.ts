...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const paymentId = (req.query.id as string)?.trim();
    ...
    console.log(`[check-payment] 🔍 Consultando Mercado Pago API: ${mpUrl}`);
    ...
    if (!mpResponse.ok) {
      console.warn(`[check-payment] ⚠️ Erro retornado pelo MP para ID ${paymentId}:`, data.message || data);
    ...
    console.log(`[check-payment] ✅ Pagamento #${paymentId} Status: ${data.status}`);
    ...
    console.error('[check-payment] ❌ Exceção ao checar pagamento:', err);
    return res.status(500).json({
      success: false,
      error: err.message || 'Erro interno ao consultar status do pagamento.',
    });
  }
}
